...
template<class Driver>
void
ScreenDriver<Driver>::DisplayText(const uint16_t x, const uint16_t y, const std::string_view text)
{
  const String arduino_string(text.data(), text.length());
  ...
  int16_t tbx, tby;
  uint16_t tbw, tbh;
  m_display.getTextBounds(arduino_string, 0, 0, &tbx, &tby, &tbw, &tbh);
  ...
  do {
    ...
    m_display.print(arduino_string);
  } while (m_display.nextPage());
}

template<class Driver>
void
ScreenDriver<Driver>::DisplayTextOnRow(const uint16_t row, const std::string_view text)
{
}